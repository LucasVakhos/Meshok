...
namespace NewsMaker
{
    public partial class NewsMakerForm : XtraForm, IAppForm, IMainForm, IPagesFrame
    {
        ...
        private void SendMe()
        {
            ...
            {
                DlgHelper.DlgWarning("??? ?????????? ??? ????????!!!");
                return;
            }
            MailSender mail = new MailSender();
            List<Task<SendCallBack>> tasks = new List<Task<SendCallBack>>();
            mail.Add(-1, "?????? ???????????", AppContextNM.MyEmail, appName + " - bug report", memoReport.Text);
            ...
            if (tasks[index].Result.Result)
                ...
                DlgHelper.DlgError("????????? ?? ?????????? " + tasks[index].Result.ErrorMess);
        }
