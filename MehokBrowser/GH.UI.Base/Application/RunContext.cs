...
using MehokBrowser.Forms;
using MeshokBrowser;
using MehokBrowser.UI.Config;
using CfgForm = MehokBrowser.UI.Config.CfgForm;
using IniHelper = LB.Libs.IniHelper;
using NotImplemented = LB.Libs.NotImplemented;
using IAppForm = MehokBrowser.UI.Interfaces.IAppForm;
...
namespace MehokBrowser.Application
{
    public class RunContext : ApplicationContext
    {
            ...
            set
            {
                ...
                if (value != null)
                {
                    if (value is IAppForm appForm)
                        appForm.InitForm();
                    AppCfg.AttachForm(value);
                    // IniHelper.AddInstance — internal; жизненный цикл управляется через CfgConnection
                }
            }
        }
        ...
        {
            Instance = this;
            // VersionHelper.CheckNewVersion(); // Заглушка: VersionHelper из GH.Components не мигрирован
            InitializeSomething();
        }
