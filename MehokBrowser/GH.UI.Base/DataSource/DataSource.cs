...
using AbstractEntity = LB.Libs.AbstractEntity;
using BaseEntity = LB.Libs.BaseEntity;
using DlgHelper = GH.Components.DlgHelper;
using MehokBrowser.UI.Interfaces;
using LB.Libs;
using DataState = GH.Components.DataState;
using GHEvents = GH.Components.GHEventsAttribute;
using DataSourceListEditor = MehokBrowser.Controls.DataSourceListEditor;
namespace MehokBrowser.Controls
{
    ...
    public partial class DataSource : BindingSource, ISupportInitialize
    {
            ...
            if (ColQty != null)
            {
                ...
                foreach (GridColumn item in view.Columns)
                {
                    ...
                    if (item == ColQty)
                    {
                        ...
                        // EdQty edQty = new EdQty(this); // stubbed
                    }
                }
            }
            if (Grid != null)
        ...
        private void CreateAction(EditTypes type, int pos) { }
        ...
        public void ExportData(string toFolder = null)
        {
            if (Grid != null)
            {
                ...
                e.FileName = Path.Combine(System.Windows.Forms.Application.StartupPath, toFolder, string.Format("{0}.xls", e.FileName));
